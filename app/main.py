...
Usage:
    python app/main.py                    # Run full pipeline
    python app/main.py --query "frage"    # Ask a question
"""
import logging
import uuid
import argparse
...
def process_document(file_path: Path, processed_dir: Path) -> Dict[str, Any]:
    ...
    # Step 2: Create document ID and enrich metadata
    document_id = str(uuid.uuid4())
    parsed_data = parsed.to_dict()
    parsed_data["document_id"] = document_id
    parsed_data["module_name"] = ""  # TODO: infer from folder structure
    parsed_data["document_type"] = ""  # TODO: infer from content

    # Step 3: Save parsed document
...
def index_chunks(config: Dict[str, Any]) -> None:
    ...
    logger.info("-" * 50)
    logger.info("Step 7: Indexing chunks into ChromaDB...")

    chunks = load_chunks_for_indexing(config["processed_path"])

    if not chunks:
        logger.warning("No chunks found for indexing. Run pipeline first.")
        return
...
def run_pipeline():
    """
    Main entry point for the Sprint 2 RAG pipeline.
    """
    logger.info("=" * 50)
    ...
    # Load configuration
    config = load_config()
    logger.info(f"Raw path: {config['raw_path']}")
    logger.info(f"Processed path: {config['processed_path']}")
    logger.info(f"Chroma path: {config['chroma_path']}")

    # Validate raw path exists
    if not config["raw_path"].exists():
        logger.error(f"Raw folder does not exist: {config['raw_path']}")
        logger.info("Please create the folder and add study documents.")
        return

    # Step 1: Scan for supported files
    logger.info("-" * 50)
    logger.info("Step 1: Scanning for supported files...")
    files = scan_intake()
    ...
    logger.info("-" * 50)
    logger.info("Step 2-6: Processing documents...")

    processed_dir = config["processed_path"]
    results = []

    for file_path in files:
        result = process_document(file_path, processed_dir)
    ...
    logger.info(f"Outputs saved to: {processed_dir}")
...
def ask_question(config: Dict[str, Any], question: str) -> None:
    ...
    for src in result["sources"]:
        logger.info(f"  - {src['source']} (score: {src['score']})")
